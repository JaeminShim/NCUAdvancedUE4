...
ADarkWarriorCharacter::ADarkWarriorCharacter()
{
	...
	LevelUpEffect->AttachParent = GetCapsuleComponent();
	static ConstructorHelpers::FObjectFinder<UParticleSystem> P_LevelUp(TEXT("/Game/InfinityBladeEffects/Effects/FX_Mobile/combat/P_levelUp_Untyped"));
	LevelUpEffect->SetTemplate(P_LevelUp.Object);
	LevelUpEffect->bAutoActivate = false;

	OnTakePointDamage.AddDynamic(this, &ADarkWarriorCharacter::PointDamageEvent);

	CharacterClass = TEXT("DarkWarrior");

	OnLevelUp.BindUObject(this, &ADarkWarriorCharacter::LevelUpAction);
}
...
//void ADarkWarriorCharacter::SetLevel(int32 NewLevel)
//{
//	Super::SetLevel(NewLevel);
//
//	if (LevelUpEffect != nullptr)
//		LevelUpEffect->Activate(true);
//}

void ADarkWarriorCharacter::LevelUpAction()
{
	if (LevelUpEffect != nullptr)
		LevelUpEffect->Activate(true);
}
